  ...
  const handleTransformApply = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Add transformation to logs
        const transformationName = TRANSFORMATIONS[selectedTransformation]?.name || selectedTransformation;
        setLogs(prev => [...prev, {
          timestamp: new Date().toISOString(),
          action: 'Transform',
          details: `Applied ${transformationName} to ${selectedParameter}, created ${createdColumns.join(', ')}`
        }]);
  ...
  const handleSave = async (fileName) => {
    try {
      ...
      setLogs(prev => [...prev, {
        timestamp: new Date().toISOString(),
        action: 'Save',
        details: `Saved dataset as ${fileName}.csv`
      }]);
